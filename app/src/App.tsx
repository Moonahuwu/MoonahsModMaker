...
/** Reference docs: every particle function class with what it does and the
 *  values vanilla uses (generated by tools/particle-catalog). */
const PARTICLE_GUIDE = "particleguide";
...
export default function App() {
  ...
  // Particle Guide deep link: class to scroll to + expand on next render.
  const [guideFocus, setGuideFocus] = useState<string | null>(null);
  // Effect Inspector modal (outline any particle; ref null = start on search).
  const [inspector, setInspector] = useState<{ open: boolean; ref: string | null }>({
    open: false,
    ref: null,
  });
  ...
  const tabs = useMemo(() => {
    ...
    // The guide is read-only docs (nothing compiles from it), so it shows
    // whenever Effects does OR on its own for editor users - always.
    ...
    <div className="flex h-screen overflow-hidden">
      ...
      <main className="relative flex h-screen min-w-0 flex-1 flex-col">
        ...
        {activeTab === MOD_COMBINER ? (
          ...
          <ParticleReference
            accent="#818cf8"
            focusClass={guideFocus}
            onFocusHandled={() => setGuideFocus(null)}
            onOpenInspector={() => setInspector({ open: true, ref: null })}
          />
        ) : activeTab === UIMASTER ? (
          <UiMasterTab
            settings={settings}
      ...
      {/* Effect Inspector: outline any particle, jump to Guide docs, open the
          CSDK editor on it. Opened from the Guide tab or an Effects row. */}
      {inspector.open && (
        <ParticleOutline
          initialRef={inspector.ref}
          helperPath={settings.vpkHelperPath}
          pakPath={settings.deadlockPak}
          csdkRoot={settings.csdkRoot}
          viewerPath={settings.source2ViewerPath}
          accent="#818cf8"
          onClose={() => setInspector({ open: false, ref: null })}
          onOpenGuide={(cls) => {
            setInspector({ open: false, ref: null });
            setActiveTab(PARTICLE_GUIDE);
            setGuideFocus(cls);
          }}
        />
      )}
